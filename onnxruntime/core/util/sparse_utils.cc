...
common::Status DenseTensorToSparseCsr(const DataTransferManager& data_manager, const Tensor& src,
  ...
  ORT_RETURN_IF_ERROR(dst_tensor.RepBuilder<SparseCsrcBuilder>().GetOrCreate(SparseCsrcFormatRep::kRowMajor,
...
Status DenseTensorToSparseCoo(const DataTransferManager& data_manager, const Tensor& src,
  ...
  ORT_RETURN_IF_ERROR(dst_result.RepBuilder<SparseCooBuilder>().GetOrCreate(linear_index, rep));
  if (nnz > 0) {
